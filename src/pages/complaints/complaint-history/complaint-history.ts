...
export class ComplaintHistoryPage {
  ...
  getComplaintHistory(type)
  {
    ...
    this.flag=0;
    this.filter.limit = 0;
    this.dbService.onPostRequestDataFromApi( {'customer_id':this.dbService.userStorageData.id ,'filter':this.filter,type:{type:type}},'app_karigar/getComplaintList', this.dbService.rootUrl).subscribe(response =>
      {
